...
bool checkValid() {
  return (JOINT1_LOW <= roll && roll <= JOINT1_HIGH && JOINT3_LOW <= pitch && pitch <= JOINT3_HIGH && GRIBBER_LOW <= roll2 && roll2 <= GRIBBER_HIGH && JOINT2_LOW <= pitch2 && pitch2 <= JOINT2_HIGH) ? true : false;
}
